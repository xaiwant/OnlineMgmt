...
if (isset($_POST["mob_number"]) && isset($_POST["email"])) {
    ...
    if (mysqli_num_rows(mysqli_query($conn, $sql)) == 0) {
		...
		if ($_POST["user_role"] == 'patient') {
          $patient_id = 'PAT' . substr(strip_tags($_POST["mob_number"], -4)) . '' . substr(strip_tags($_POST["firstname"], 0, 2)) . '' . substr(strip_tags($_POST["lastname"], 0, 2)) . '' . substr(strip_tags($_POST["DatOfBirth"], 0, 2)) . '' . date('His', time());
		} else {
		  $patient_id = 'DOC' . substr(strip_tags($_POST["mob_number"], -4)) . '' . substr(strip_tags($_POST["firstname"], 0, 2)) . '' . substr(strip_tags($_POST["lastname"], 0, 2)) . '' . substr(strip_tags($_POST["DatOfBirth"], 0, 2)) . '' . date('His', time());	
		}
        ...
        $query      = "INSERT INTO Users (patientid, firstname, middlename, lastname, DatOfBirth, gender, mob_number, email, pass1, pass2, user_role, blood_group, height, weight, createdtime,secuirtyq, secuirtya )
						VALUES ('" . strtoupper($patient_id) . "', '" . strip_tags($_POST["firstname"]) . "','" . strip_tags($_POST["middlename"]) . "','" . strip_tags($_POST["lastname"]) . "','" . $_POST["DatOfBirth"] . "','" . $_POST["gender"] . "',
						'" . strip_tags($_POST["mob_number"]) . "','" . strip_tags($_POST["email"]) . "','" . strip_tags(md5($_POST["pass1"])) . "','" . strip_tags(md5($_POST["pass2"])) . "','" . $_POST["user_role"] . "','" . $_POST["blood_group"] . "','" . strip_tags($_POST["height"]) . "',
						'" . strip_tags($_POST["weight"]) . "', '" . time() . "', '" . $_POST["secuirtyq"] . "', '" . strip_tags($_POST["secuirtya"]) . "')";
